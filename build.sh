...
./autogen.sh \
&& ./configure CC=gcc-4.4 --with-wsf --enable-openssl --with-axis2=`pwd`/wsf_c/axis2c/include --enable-tests=no --enable-wsclient=no --enable-savan=no --prefix=`php-config --extension-dir`/wsf_c \
&& make \
&& mkdir -p `php-config --extension-dir`/wsf_c/docs \
&& make install
